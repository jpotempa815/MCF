...
/******* funkcje *****************************/
	...
	void evolution(double,int, const char *);
...
void DSMC_2D::evolution(double time_max,int nit, const char *zad){
	int i,it=0;
	time_t czas1,czas2;
	double czas; //czas trwania obliczen na komputerze [s]
	string nazwa,katalog;
    string nazwa2, katalog2;
	string nazwa3, katalog3;
    ...
    katalog2="histogramy";
	if(!std::filesystem::exists(katalog2)){
		std::filesystem::create_directory(katalog2);
	}
	katalog3="rv";
	if(!std::filesystem::exists(katalog3)){
        std::filesystem::create_directory(katalog3);
    }
	...
	while(time_sum<time_max || it<nit){
		...
		if(it == 100 || it ==500 || it == 1000 || it == 10000)
		{
			nazwa=katalog+string("/nptv_")+zad+to_string(it)+string(".dat");
			write_nptv(nazwa.c_str(),3);
            nazwa2 = katalog2 + string("/hist_")+zad+to_string(it)+string(".dat");
            hist_velocity_all(nazwa2.c_str(),5.0,50);
			nazwa3 = katalog3 + string("/rv_")+zad+to_string(it)+string(".dat");
			write_position_velocity(nazwa3.c_str());
		}
		mean_free_path();
		autocorrelation();
